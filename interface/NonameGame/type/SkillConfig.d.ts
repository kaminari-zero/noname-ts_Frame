...
interface ExSkillData {
     ...
     * 触发的时机 
     * 一般用于触发技能（被动技能）
     * 注：主动触发enable，是没有event._trigger;
     */
    trigger?: ExTriggerData;
    /**
     * 为true时，将该技能加入到_hookTrigger （理解错误，这是个map结构，目前看来是一个技能配置）
     * 具体作用尚为知晓
     */
    hookTrigger?: ExSkillData;
    /** 同时机技能发动的优先度 */
    priority?: number;
    ...
    */
    ...
    //|TwoParmFun<Links, Player, string>
    /** 
    ...
    /**
     * 显示日志的标记
     * 若值为“notarget”，则显示出对所有目标相关描述的日志
     */
    log?: string | ThreeParmFun<Player,string,Target[],void>;
    /**
     * 目标日志
