	...
	function(lib,game,ui,get,ai,_status){
		return {
			...
			content:function (config,pack){
				...
				game.cxyFindCode1 = {
					...
					find: function(str,num){
						...
						for(var i in lib.skill) {
							var info = lib.translate[i];
							if(!info)continue;
							if(typeof info != "string")continue;
							var similar = 0;
							for(var k=0;k<strs.length;k++){
								similar+=game.cxyFindCode1.compare(strs[k][0],info,strs[k][1]||"");
							}
							similar = similar/strs.length;
							if(similar>(num||0.6)) {
								tmp[i] = get.round(similar,4);
			...
			config: {
				...
				cxyFind: {
					name:"<font color=#ff4a86>按描述搜索</font>",
					clear: true,
					onclick: function(){
						try{
					...
					onclick: function(){
						try{
							...
							inputComps.button.addEventListener("click",function(){
								...
								var info = textInput.value;
								if(!info)return ;
								var infos = game.cxyFindCode1.find(info,0.5);
								/*
									排序
								*/
								var sortInfos = [] , max_similar = null;
								...
								/*
									输出
								*/
								titleComps.text.innerHTML = "按名字搜索[共查询到 "+sortInfos.length+" 条结果]"
								function print(arr){
									var tmp = ui.create.div(".cxyFindCode_result_tmp");
									var tmpComps = {
										lookCode: ui.create.div(".cxyFindCode_result_tmp_lookCode","查看代码"),
